...
function countMovie(db){
    ...
    return Promise.all([ 
        dbo.collection(process.env.CL_INFO_MOVIE).countDocuments({ $and: [ { 't_l.t_f': 0 }, { 'c_c.ca': { $nin: ["19"] } } ] }), 
        dbo.collection(process.env.CL_INFO_MOVIE).countDocuments({ $and: [ { 't_l.t_f': 1 }, { 'c_c.ca': { $nin: ["19"] } } ] }), 
        dbo.collection(process.env.CL_INFO_MOVIE).countDocuments({ $and: [ { $or: [ { 't_l.t_f': 0 }, { 't_l.t_f': 1 } ] }, { 'c_c.ca': { $in: ["19"] } } ] }), 
        dbo.collection(process.env.CL_INFO_MOVIE).countDocuments({ $and: [ { 't_l.t_f': 2 }, { 'c_c.ca': { $nin: ["19"] } } ] }), 
        dbo.collection(process.env.CL_INFO_MOVIE).countDocuments({ $and: [ { 't_l.t_f': 3 }, { 'c_c.ca': { $nin: ["19"] } } ] }), 
        dbo.collection(process.env.CL_INFO_MOVIE).countDocuments({ $and: [ { $or: [ { 't_l.t_f': 2 }, { 't_l.t_f': 3 } ] }, { 'c_c.ca': { $in: ["19"] } } ] }), 
        dbo.collection(process.env.CL_INFO_MOVIE).countDocuments({ $and: [ { $or: [ { 't_l.t_f': 0 }, { 't_l.t_f': 1 } ] }, { 'c_c.ca': { $in: ["v6"] } }, { 'c_c.ca': { $nin: ["19"] } } ] }), 
        dbo.collection(process.env.CL_INFO_MOVIE).countDocuments({ $and: [ { $or: [ { 't_l.t_f': 2 }, { 't_l.t_f': 3 } ] }, { 'c_c.ca': { $in: ["v6"] } }, { 'c_c.ca': { $nin: ["19"] } } ] }), 
        dbo.collection(process.env.CL_INFO_MOVIE).countDocuments({ $and: [ { 'c_c.ca': { $in: ["v6"] } }, { 'c_c.ca': { $in: ["19"] } } ] }) ]);
}
...
function getInfoFilm (db, db_4, id){
    ...
    return Promise.all([ dbo.collection(process.env.CL_INFO_MOVIE).findOne({ "_id" : ObjectID(id) }), dbo_4.collection(process.env.CL_CAST).find( { movie: { $in: [id] } }, { projection: { name : 1, _id : 0 } } ).toArray() ]);
}
...
function addFilm(db, params) {
    ...
    // Create array info
    let update = {}, img = {}, name = {}, status = {}, c_c = {}, content = {}, t_l = {}, other = {};
    delete params.cast;

    update['url'] = params.url ? params.url: helpers.viSeo(params.name.vn);
    img['poster'] = update['url'];
    ...
    content['s_t'] = params.content.s_t ? params.content.s_t : helpers.keyTag(`${ params.name.vn }, ${ pparams.name.en }`);
...
function addMovie2Cast(db_4, id, cast){
    ...
    cast.split(', ').forEach(element => {
        dbo_4.collection(process.env.CL_CAST).updateMany( { name: element }, { $addToSet: { movie: { $each: [ ObjectID(id).toString() ] } } }, { upsert: true } );
    });
}
// function addFilm(db, params) {
//     if(params){
...
function editFilm(db, db_4, params) {
    ...
    const dbo_4 = db_4.db(process.env.DB_SV_4);
    if (params.cast_add) {
        dbo_4.collection(process.env.CL_CAST).updateMany( { name: { $in: params.cast_add } }, { $addToSet: { movie: { $each: [ params._id ] } } }, { upsert: true } );
    }
    if (params.cast_del) {
        dbo_4.collection(process.env.CL_CAST).updateMany( { name: { $in: params.cast_del } }, { $pull: {  movie:  params._id  } }, { upsert: true } );
    }
    // Create array info
    let update = {}, img = {}, name = {}, status = {}, c_c = {}, content = {}, t_l = {}, other = {};
    ...
    const dbo = db.db(process.env.DB_SV_1);
    return dbo.collection(process.env.CL_INFO_MOVIE).updateOne({ "_id" : ObjectID(params._id) }, { $set: update });
}
...
function delMovie (db, id){
    if(id){
        let dbo = db.db(process.env.DB_SV_1);
        return dbo.collection(process.env.CL_INFO_MOVIE).deleteOne({ "_id" : ObjectID(id) });
    }
    return false;
}
// * DELETE MOVIES * // 
function delMovies (db, arrayID){
    ...
    const dbo = db.db(process.env.DB_SV_1);
    return dbo.collection(process.env.CL_INFO_MOVIE).deleteMany({ "_id" : { $in: arrayObjectID } });
}
// * ADVANCE MOVIES * // 
function advanceMovie (db, type, arrayID){
    ...
    switch (type) {
        case 'hinden-movie':
            return dbo.collection(process.env.CL_INFO_MOVIE).updateMany( { "_id" : { $in: arrayObjectID } }, { $set: { 't_l.t_so': 1 } });
        case 'del-hinden':
            return dbo.collection(process.env.CL_INFO_MOVIE).updateMany( { "_id" : { $in: arrayObjectID } }, { $unset: { 't_l.t_so': 1 } } );
        case 'add-movie-sl':
            return dbo.collection(process.env.CL_INFO_MOVIE).updateMany( { "_id" : { $in: arrayObjectID } }, { $set: { 't_l.t_s': 1 } });
        case 'del-movie-sl':
            return dbo.collection(process.env.CL_INFO_MOVIE).updateMany( { "_id" : { $in: arrayObjectID } }, { $unset: { 't_l.t_s': 1 } } );
        case 'add-movie-cr':
            return dbo.collection(process.env.CL_INFO_MOVIE).updateMany( { "_id" : { $in: arrayObjectID } }, { $set: { 't_l.t_r': 1 } });
        case 'del-movie-cr':
            return dbo.collection(process.env.CL_INFO_MOVIE).updateMany( { "_id" : { $in: arrayObjectID } }, { $unset: { 't_l.t_r': 1 } } );
        case 'pin-movie':
            return dbo.collection(process.env.CL_INFO_MOVIE).updateMany( { "_id" : { $in: arrayObjectID } }, { $set: { 't_l.t_p': 1 } } );
        case 'unpin-movie':
        return dbo.collection(process.env.CL_INFO_MOVIE).updateMany( { "_id" : { $in: arrayObjectID } }, { $unset: { 't_l.t_p': 1 } } );
        default:
            return new Promise((resolve, reject) => {
                reject({ code: 0 })
            })
...
function CURDEpisode(db, params) {
    ...
    const dbo = db.db(process.env.CL_EPISODE);
    return dbo.collection(process.env.CL_EPISODE).updateOne( { "_id" : ObjectID(params.id) }, { $set: { l: JSON.parse(params.episode) } }, { upsert: true } );

    // if (params) {
    //     let defer = q.defer();
    //     let dbo = db.db(process.env.CL_EPISODE), data = params.listEp.replace(/[\s]/gi, '').split('/view'), episode = [];
    ...
    if (params) {
        return new Promise((resolve, reject) => {
            ...
            dbo.collection(process.env.CL_EPISODE).findOne({ "_id" : ObjectID(params.id) }, (err, result) => {
                ...
                resolve(dbo.collection(process.env.CL_EPISODE).updateOne( { "_id" : ObjectID(params.id) }, { $set: { l: episode } }, { upsert: true } ));
            });
        });
    }
    return false;
...
function addEpisodeOne(db, params) {
    if (params) {
        ...
        let dbo = db.db(process.env.CL_EPISODE);
        return dbo.collection(process.env.CL_EPISODE).updateOne( { "_id" : ObjectID(params.id) }, { $push: { l: episode } }, { upsert: true } );
    }
    return false;
}
...
function editEpOne(db, params){
    if (params) {
        let episode = {}, dbo = db.db(process.env.CL_EPISODE), update = {};
        episode['t'] = params.name;
        params.drive ? episode['d'] = helpers.getIDDrive(params.drive) : episode;
        params.photo ? episode['p'] = helpers.trimStr(params.photo): episode;
        params.embed ? episode['e'] =  helpers.trimStr(params.embed) : episode;
        update['l.'+params.value] = episode;
        return dbo.collection(process.env.CL_EPISODE).updateOne( { "_id" : ObjectID(params.id) }, {$set: update}, { upsert: true } );
    }
    return false;
}
...
function delEpOne(db, params){
    if (params) {
        let episode = {}, dbo = db.db(process.env.CL_EPISODE);
        episode['t'] = params.name;
        params.drive ? episode['d'] = helpers.getIDDrive(params.drive) : episode;
        params.photo ? episode['p'] = helpers.trimStr(params.photo): episode;
        params.embed ? episode['e'] =  helpers.trimStr(params.embed) : episode;
        return dbo.collection(process.env.CL_EPISODE).updateOne( { "_id" : ObjectID(params.id) }, {$pull: {l: episode}}, { upsert: true } );
    }
    return false;
}
...
function delAllEpisode(db, id){
    ...
    const dbo = db.db(process.env.CL_EPISODE);
    return dbo.collection(process.env.CL_EPISODE).deleteOne({ "_id" : ObjectID(id) });
}
